...
if [ ! -d /var/run/lightdm/root/ ]; then
	...
	x11vnc -ncache 10 -forever -shared -capslock -rfbport 5900 >> $LOGPATH &
else
	# If this is ubuntu filesystem.
	x11vnc -ncache 10 -forever -shared -capslock -rfbport 5900 -auth /var/run/lightdm/root/:0 -display :0 >> $LOGPATH &
fi
